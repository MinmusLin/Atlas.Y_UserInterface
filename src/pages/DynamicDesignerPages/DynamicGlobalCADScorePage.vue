      ...
      <div class='title-section'>
        ...
        <div class='subtitle'>
          <span style='margin-right: 50px'>
            <strong>Sensor Protein: </strong>{{ currentResult.dfpId }}
          </span>
          <span style='margin-right: 50px'>
            <strong>Sensitive Area: </strong>{{ g_report_dynamic[1] }}
          </span>
          <span style='margin-right: 50px'>
            <strong>Linker: </strong>{{ currentResult.linkerId }}
          </span>
          <span>
            <strong>Fluorescent Tag: </strong>{{ g_report_dynamic[8] }}
          </span>
        </div>
      </div>
      ...
      <div class='content-section'>
        ...
        <div class='right-section'>
          ...
          <DefaultButton width='488px'
                         ...
                         @click="router.push(`/dynamic-designer/local-cad-score/${fpId}`)"
                         :active='true'/>
        </div>
      </div>
    </div>
...
const fetchPdb = async () => {
  try {
    const response = await axiosInstance.get(`/dynamic-prediction/get-file/${g_queryLogId_dynamic.value}/pdb/${currentResult.value.dfpId}.pdb`)
...
const fetchScore = async () => {
  try {
    const response = await axiosInstance.get(`/dynamic-prediction/get-global-cad-score/${g_queryLogId_dynamic.value}/${currentResult.value.dfpId}`)
