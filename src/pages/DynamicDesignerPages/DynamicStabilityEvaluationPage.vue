  ...
  <div class='page-container'>
    ...
    <div class='subtitle-container'>
      <span class='subtitle' style='margin-right: 50px'>
        <span class='sub-content'>Sensor Protein: </span>{{ currentResult.dfpId }}
      </span>
      <span class='subtitle' style='margin-right: 50px'>
        <span class='sub-content'>Sensitive Area: </span>{{ g_report_dynamic[1] }}
      </span>
      <span class='subtitle' style='margin-right: 50px'>
        <span class='sub-content'>Linker: </span>{{ currentResult.linkerId }}
      </span>
      <span class='subtitle'>
        <span class='sub-content'>Fluorescent Tag: </span>{{ g_report_dynamic[8] }}
      </span>
    </div>
...
const fetchStability = async () => {
  try {
    const response = await axiosInstance.get(`/dynamic-prediction/get-stability/${g_queryLogId_dynamic.value}/pdb/${currentResult.value.dfpId}.pdb`)
    for (let i = 0; i < 20; i++) {
